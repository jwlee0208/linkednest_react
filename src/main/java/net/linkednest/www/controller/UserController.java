...
public class UserController {
    ...
    @Operation(summary = "회원 등록"
             ...
             , responses = {
            @ApiResponse(responseCode = "200", description = "회원가입 성공", content = @Content(schema = @Schema(implementation = ResUserRegistDto.class)))
    })
    public ResponseEntity registUser(@RequestBody(required = true, description = "회원가입 요청 파라미터") ReqUserRegistDto reqUserRegistDto) {
    ...
    @Operation(summary = "회원 로그인"
            ...
            , tags = {"User Controller"}
            , responses = {
            @ApiResponse(responseCode = "200", description = "회원 로그인 성공", content = @Content(schema = @Schema(implementation = ResUserLoginDto.class)))
    })
